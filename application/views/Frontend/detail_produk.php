...
<body>
    ...
    <div class="container">
        <div class="navigation">
            ...
            <a
                href="<?= base_url(); ?>homepage/detail_product/<?= $product['id_produk']; ?>"><?= $product['nama_produk']; ?></a>
            <i class="fa fa-caret-right"></i>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-sm-4" style="background-color:lavenderblush;">
                    <div class="img">
                        <a href="<?= base_url(); ?>assets/Gambar/foto_produk/<?= $product['gambar_produk']; ?>"
                            data-lightbox="img-1">
                            <img src="<?= base_url(); ?>assets/Gambar/foto_produk/<?= $product['gambar_produk']; ?>"
                                alt="produk" class="jumbo-thumb">
                        </a>
                        <div class="img-slider">
                            ...
                            <img src="<?= base_url(); ?>assets/Gambar/foto_produk/<?= $d['foto']; ?>" alt="gambar"
                                class="thumb">
                            <?php endforeach; ?>
                        </div>
                    </div>
                ...
                <div class="col-sm-6">
                    <nav class="navbar sticky-top navbar-transparent bg-transparent">
                        <div class="card border-white">
                            <form action="<?= base_url('keranjang/save_keranjang_beli') ?>" method="POST">
                                <div class="ket">
                                    ...
                                    <table class="table table-borderless">
                                        ...
                                        <tr>
                                            <td>Harga</td>
                                            <input type="hidden" id="harga" name="harga" onkeyup="OnChange(this.value)"
                                                onKeyPress="return isNumberKey(event)"
                                                value="<?= $product['harga']; ?>" />
                                            <td style="text-align:center" class="price">Rp
                                        ...
                                        <tr class="newPrice">
                                            <td>Promo</td><input type="hidden" id="harga" name="harga"
                                                onkeyup="OnChange(this.value)" onKeyPress="return isNumberKey(event)"
                                                value="<?= $product['hargadiskon']; ?>" />
                                            <td style="text-align:center" class="price">Rp
                                                <?= str_replace(",", ".", number_format($product['hargadiskon'])); ?>
                                            </td>
                                        </tr>
                                        <?php } ?>
                                        <tr>
                                        ...
                                        <tr>
                                            ...
                                            <td><select id="attribut" name="attribut"
                                                    style="height: 40px; font-size: medium;" required>
                                                    <option value="" selected disabled> - Pilih Warna -</option>
                                                    <?php foreach ($attribut as $row) : ?>
                                                    <?php if ($row->qty >= 1) { ?>
                                                    <option value="<?php echo $row->id_attribut; ?>"
                                                        warna="<?php echo $row->warna; ?>"
                                                        stok="<?php echo $row->qty; ?>"><?php echo $row->warna; ?>
                                                    </option>
                                                    <?php } else { ?>
                                                    <option value="<?php echo $row->id_attribut; ?>" disabled
                                                        warna="<?php echo $row->warna; ?>"
                                                        stok="<?php echo $row->qty; ?>"><?php echo $row->warna; ?> (STOK
                                                        KOSONG)</option>
                                                    <?php } ?>
                                                    <?php endforeach; ?>
                                                </select>
                                        ...
                                        <tr>
                                            ...
                                            <td style="text-align:center">
                                                <input type="number" min="1" max="" placeholder="QTY" name="jumlah"
                                                    id="jumlah" onchange="OnChange(this.value)"
                                                    onfocusout="return isNumberKey(event)" required />
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Total</td>
                                            <td>
                                                <input type="text" name="totalharga" id="total"
                                                    placeholder="Rp <?= str_replace(",", ".", number_format($product['harga'])); ?>"
                                                    disabled>
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <input type="hidden" name="id_produk"
                                                    value="<?php echo $product['id_produk'] ?>"> <!-- id Produk -->
                                                <input type="hidden" name="warna1" id="warna1" value="">
                                                <!-- warna Produk -->
                                                <input type="hidden" name="berat_produk"
                                                    value="<?php echo $product['berat_produk'] ?>">
                                                <!-- berat Produk -->
                                                <input type="hidden" name="jumlah1" id="jumlah1" />
                                                <input type="hidden" name="jumlah2" id="jumlah2" />
                                                <button type="submit" disabled class="btn btn-warning pl-5 pr-5 btn-lg"
                                                    style="font-size: 15px">Beli</button>
                                            </td>
                                            <td>
                                                <button type="submit" disabled class="btn btn-primary btn-lg"
                                                    style="font-size: 15px"
                                                    formaction="<?= base_url('keranjang/save_keranjang') ?>">Tambah ke
                                                    Keranjang</button>
                            </form>
                            </td>
                            <?php } else { ?>
                            <td>
                                <input type="hidden" name="id_pelanggan"
                                    value="<?php echo $this->session->userdata('id') ?>"> <!-- id Pelanggan -->
                                ...
                                <input type="hidden" name="berat_produk" value="<?php echo $product['berat_produk'] ?>">
                                <!-- berat Produk -->
                                <input type="hidden" name="jumlah1" id="jumlah1" />
                                <input type="hidden" name="jumlah2" id="jumlah2" />
                                <button type="submit" class="btn btn-warning pl-5 pr-5 btn-lg"
                                    style="font-size: 15px">Beli</button>
                            </td>
                            <td>
                                <button type="submit" class="btn btn-primary btn-lg" style="font-size: 15px"
                                    formaction="<?= base_url('keranjang/save_keranjang') ?>">Tambah ke
                                    Keranjang</button>
                                </form>
                            </td>
    ...
    $("select[name=attribut]").on("change", function() {
        ...
        var stok_warna = $("option:selected", this).attr(
            'stok'); // Menangkap Inputan Dari Dropdown dari atribut stok
        var warna = $("option:selected", this).attr(
            'warna'); // Menangkap Inputan Dari Dropdown dari atribut warna
        //alert(stok_warna);
        $("#stok").html(stok_warna);
        //$("#max").html(stok_warna);
        document.getElementById("jumlah").max =
            stok_warna; // Mengambil Inputan Dari Dropdown dan mengirimnya ke Tag Input Atribut max
