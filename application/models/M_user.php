...
public function getOrder(){
  ...
  {
    $id = $this->session->userdata('id');
    $hasil = $this->db->query("UPDATE `tbl_pelanggan` SET `nama_pelanggan` = '$nama', `no_pelanggan`='$no', `alamat_pelanggan`='$alamat', `kode_pos`='$kodepos', `kabupaten`='$kab', `provinsi`='$prov', `password_pelanggan`='$pwd' WHERE `tbl_pelanggan`.`id_pelanggan` = '$id';");
    return $hasil;
  }
}
